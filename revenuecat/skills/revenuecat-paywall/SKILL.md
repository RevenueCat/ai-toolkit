...
- **RevenueCatUI renders ALL paywall UI.** Do not author custom paywall, upsell, or fallback purchase screens alongside it — the only permitted custom view is a minimal "paywall unavailable" state (a short message, no product list, no purchase buttons). If you find yourself building plan cards or price rows, stop: either the dashboard paywall needs fixing, or the app wants the custom-UI path (a different skill) — ask.
...
2. The paywall exists, is published, and is attached to the intended offering; record it by both name and id, e.g. `Moodly Pro (pw_abc123)`.
