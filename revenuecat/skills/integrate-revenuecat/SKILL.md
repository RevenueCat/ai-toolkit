...
- **RevenueCat Paywall (recommended default).** Designed in the dashboard and rendered by RevenueCatUI. Design, copy, and packages change without an app release, and paywall variants can be A/B tested with RevenueCat Experiments. Create it with `create-paywall-ai` (MCP) or `rc paywalls generate` (CLI), attached to the current offering (see `revenuecat-paywall-design`), then present it with `revenuecat-paywall`.
