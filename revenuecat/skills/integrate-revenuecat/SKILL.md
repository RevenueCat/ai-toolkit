...
- **RevenueCat Paywall (recommended default).** Hosted by RevenueCat and rendered by RevenueCatUI. You create it yourself with `create-paywall-ai` (MCP) or `rc paywalls generate` (CLI), attached to the current offering and informed by the app's codebase (see `revenuecat-paywall-design`), then present it with `revenuecat-paywall`. The user does not have to design anything in the dashboard; they can refine it later in the paywall builder. Design, copy, and packages change without an app release, and paywall variants can be A/B tested with RevenueCat Experiments.
